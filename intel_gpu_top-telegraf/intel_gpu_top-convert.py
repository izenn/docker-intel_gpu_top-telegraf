...
def insert_data(msg, ignorelist):
  msg = json.loads(msg)
  for key, value in flattening(msg, "", ignorelist).items():
    outkey = key.replace(".", "=", 1).replace(".", " ")
    outvalue = value
    if not isinstance(value, (int, float)):
      outvalue = "\""+value+"\""
    print("intel_gpu_top,",outkey,"=",outvalue, sep='')
  
data = sys.stdin.read()
data = '{"card":'+data
data = data.replace("duration","length")
data = data.replace(',"engines"', '},"engines"')
